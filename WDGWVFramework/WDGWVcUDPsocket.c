...
#include <unistd.h>
#include <netdb.h>
#define WDGWVcUDPsocket_buff_len 8192

//return socket fd
int WDGWVcUDPsocket_server(const char *address, int port) {
...
int WDGWVcUDPsocket_recive(int socket_fd, char *outdata, int expted_len, char *remoteip, int *remoteport) {
...
int WDGWVcUDPsocket_close(int socket_fd) {
    return close(socket_fd);
}

//return socket fd
int WDGWVcUDPsocket_client() {
    //create socket
    int socketfd = socket(AF_INET, SOCK_DGRAM, 0);
    int reuseon = 1;
...
int WDGWVcUDPsocket_get_server_ip(char *host, char *ip) {
    struct hostent *hp;
...
int WDGWVcUDPsocket_sentto(int socket_fd, char *msg, int len, char *toaddr, int topotr) {
    struct sockaddr_in address;
